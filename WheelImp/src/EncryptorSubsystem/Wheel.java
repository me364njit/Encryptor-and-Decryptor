...
public class Wheel {
  ...
  private void loadLToR() {
    ...
    for (int i = 0; i < nRec; ++i) lToR.add(i);
    for (int i = 0; i < nRec; ++i) {
      ...
      // number (i.e., lToR[i] = i.
      //
      while ((i == lToR.get(j)) || (i == lToR.get(j + 1))) j = r.nextInt(rRange);
    ...
    if (DEBUG) {
      ...
      for (int i = 0; i < nRec; ++i) {
        boolean found = false;
        for (int j = 0; j < nRec && (found == false); ++j) found = (j == lToR.get(i));
        if (!found) {
          System.out.print(i);
  ...
  private void loadRToL() {
    ...
    if (DEBUG)
      for (int i = 0; i < nRec; ++i) {
        boolean found = false;
        for (int j = 0; j < nRec && (found == false); ++j) found = (j == rToL.get(i));
        if (!found) {
          System.out.print(i);
          System.out.print(" NOT FOUND ");
          System.out.print("  ");
  ...
  // nPos = notch position
  //
  public Wheel(int nEnts, long seedIn, int cPos, int nPos) {
    nRec = nEnts;
    notchLoc = nPos;
    curPos = cPos;
    seed = seedIn;
